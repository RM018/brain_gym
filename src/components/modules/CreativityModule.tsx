        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
            <div className="bg-[#041517]/60 border border-emerald-400/30 p-4 sm:p-6 rounded-xl text-center">
              <div className="text-xs sm:text-sm text-gray-400 mb-2">Overall Creativity</div>
            ...
            <div className="bg-[#041517]/60 border border-emerald-400/30 p-4 sm:p-6 rounded-xl">
              <div className="text-xs sm:text-sm text-gray-400 mb-3 sm:mb-4">Creativity Breakdown</div>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs sm:text-sm">Semantic Distance:</span>
                  <span className="font-bold text-blue-400 text-sm sm:text-base">
                    {(results.profile.semanticDistance * 100).toFixed(0)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs sm:text-sm">Divergent Thinking:</span>
                  <span className="font-bold text-purple-400 text-sm sm:text-base">
                    {results.profile.divergentThinking.toFixed(0)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs sm:text-sm">Originality:</span>
          ...
          <div className="bg-[#041517]/60 border border-emerald-400/30 p-4 sm:p-6 rounded-xl mb-4 sm:mb-6">
            <div className="text-base sm:text-lg font-bold mb-3 sm:mb-4 text-emerald-300">Your Words:</div>
            <div className="flex flex-wrap gap-2 sm:gap-3">
              {words.map((word, i) => (
                <motion.div
                  ...
                  className="px-3 sm:px-4 py-1.5 sm:py-2 bg-[#041517]/80 border border-emerald-400/40 rounded-full text-sm sm:text-base md:text-lg"
                >
                  {word}
                </motion.div>
              ))}
          ...
          <div className="bg-green-500/10 border border-green-400/30 p-4 sm:p-6 rounded-xl">
            <div className="font-bold mb-2 sm:mb-3 text-emerald-300 text-sm sm:text-base">Insights:</div>
            <ul className="space-y-1.5 sm:space-y-2">
              {results.profile.insights.map((insight: string, i: number) => (
                <li key={i} className="text-gray-300 text-xs sm:text-sm md:text-base">• {insight}</li>
              ))}
            </ul>
          </div>
        </motion.div>
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-slate-900/95 to-teal-900/40 p-3 sm:p-4 md:p-6">
      <div className="w-full max-w-3xl">
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
          <div className="bg-[#041517]/80 border border-purple-400/40 px-4 sm:px-6 py-2 sm:py-3 rounded-full">
            <span className="text-purple-400 font-bold text-sm sm:text-base">WORDS: </span>
            <span className="text-xl sm:text-2xl font-mono text-white">{filledWords}/10</span>
          </div>
          <div className="bg-[#041517]/80 border border-blue-400/40 px-4 sm:px-6 py-2 sm:py-3 rounded-full">
            <span className="text-blue-400 font-bold text-sm sm:text-base">PROGRESS: </span>
            <span className="text-xl sm:text-2xl font-mono text-white">{((filledWords / 10) * 100).toFixed(0)}%</span>
          </div>
        </div>

        {/* Progress Bar */}
        <div className="bg-[#041517]/60 border border-emerald-400/20 rounded-full h-2 sm:h-3 mb-6 sm:mb-8 overflow-hidden">
          <motion.div
        ...
        {/* Input Grid */}
        <div className="bg-[#0a2024]/80 border border-emerald-500/40 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 mb-4 sm:mb-6">
          <div className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-center text-purple-400">
            ...
            {words.map((word, index) => (
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="text-lg sm:text-xl md:text-2xl font-bold text-blue-400 min-w-[24px] sm:min-w-[28px]">{index + 1}</div>
                  <input
                    type="text"
                    value={word}
                    ...
                    className={`flex-1 px-3 sm:px-4 py-2 sm:py-3 bg-[#041517]/80 border-2 rounded-lg sm:rounded-xl text-white placeholder-gray-500 focus:outline-none transition-colors text-sm sm:text-base ${
        ...
        {errors.length > 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="bg-red-500/20 border border-red-400/40 p-3 sm:p-4 rounded-xl mb-4 sm:mb-6"
          >
            <div className="text-red-400 font-bold mb-2 text-sm sm:text-base">Please fix:</div>
            <ul className="space-y-1">
              {errors.map((error, i) => (
                <li key={i} className="text-red-300 text-xs sm:text-sm">• {error}</li>
              ))}
            </ul>
          </motion.div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          ...
          <motion.button
            ...
            className="flex-1 px-8 sm:px-12 py-3 sm:py-4 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full text-base sm:text-lg md:text-xl font-bold disabled:opacity-50 disabled:cursor-not-allowed hover:from-purple-600 hover:to-pink-600 disabled:hover:from-purple-500 disabled:hover:to-pink-500"
          >
            {filledWords < 10 ? `${10 - filledWords} more word${10 - filledWords === 1 ? '' : 's'} needed` : 'ANALYZE CREATIVITY'}
          </motion.button>
        </div>
      </div>
    </div>
